...
> **v1.2.8**：修复"**关机重启后自动启动**时 UI 界面文字位置对不齐"（手动打开正常，仅开机自启时机出现）。
> 根因：主窗口默认 `show:true` 且无 `ready-to-show` 门控——页面仍在加载/布局时窗口就已可见。
> 手动启动机器空闲、首帧瞬间完成故无感；但**开机自启**时机下系统字体（Microsoft YaHei）尚未度量完成，
> 首帧退到回退字体做布局，行高/升降部度量不同 → `el-table` 行高、列边界、文本基线"对不齐"。
> 修复（与悬浮框 `show:false → 加载完再显示` 的做法对齐）：
> ① **渲染层**（`core/app-main.ts`）：`document.fonts.ready` 之后再 `app.mount('#app')`，保证"首次布局"即用真实字体度量，从根上避免错位（字体异常时该 Promise 仍 resolve，不卡启动）；
> ② **主进程**（`core/main.ts`）：窗口 `show:false`，`ready-to-show` 后再 `show()`，并加 3s 兜底强制显示（防极端情况窗口不出现）。
>
> **v1.2.7**：四项完善——
