        ...
        <el-card>
          ...
          <el-form :model="form" label-width="90px" :rules="rules" ref="formRef">
            ...
            <el-form-item>
              ...
              <span class="muted">创建后自动预置 6 个阶段（可增删/改名/调序）</span>
            </el-form-item>
          </el-form>
        </el-card>
      </el-col>
...
import { Refresh, Plus, List } from '@element-plus/icons-vue';
...
async function create() {
  ...
  creating.value = true;
  try {
    const project = createDefaultProject({ ...form });
    const { folder } = await window.api.createProject(rootDir.value, project);
    ElMessage.success(`项目已创建：${project.info.name}`);
